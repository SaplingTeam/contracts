...
contract SaplingProtocolPool is SaplingPoolContext {
    ...
    function invest(address lendingPool, uint256 liquidityTokenAmount) external onlyManager whenNotPaused {
        require(isPoolFunctional(), "SaplingProtocolPool: invalid pool state");
        require(strategyLiquidity() >= liquidityTokenAmount, "SaplingProtocolPool: insufficient liquidity");
        require(
            IVerificationHub(verificationHub).isSaplingPool(lendingPool),
            "SaplingProtocolPool: unregistered lending pool"
        );
        ...
        weightedAvgStrategyAPR = prevStrategizedFunds
            ...
            .add(liquidityTokenAmount.mul(investmentAPR))
    ...
    function collectYield(address pool, uint256 amount) external onlyManager whenNotPaused {
        require(poolYieldBalanceOn(pool) >= amount, "SaplingProtocolPool: insufficient yield balance");
    ...
    function collectInvestment(address pool, uint256 amount) external onlyManager whenNotPaused {
        ...
        require(
            0 < amount && amount <= investment.outstandingAmount.add(poolYieldBalanceOn(pool)),
            "SaplingProtocolPool: invalid amount"
        );

        /*
         External pool liquidity and balance checks are not necessary here as withdraw() call will fail on insufficient
         liquidity or balance, and the pool liquidity and balance can be verified independently.
        */
