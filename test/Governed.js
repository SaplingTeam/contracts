...
describe("Governed (SaplingPool)", function () {
  ...
  beforeEach(async function () {
    ...
    let tokenContract = await TestUSDC.deploy("Test USDC", "TestUSDC", 6);
  ...
  describe("Initial state", function () {
    it("Protocol governance address is correct", async function () {
      expect(await poolContract.governance()).to.equal(
        currentGovernance.address
      );
    });
  ...
  describe("Transfer Governance", function () {
    it("Can transfer", async function () {
      await poolContract
        .connect(currentGovernance)
        .transferGovernance(governance2.address);
    ...
    describe("Rejection scenarios", function () {
      it("Transferring to NULL address should fail", async function () {
        await expect(
          poolContract
            .connect(currentGovernance)
            .transferGovernance(NULL_ADDRESS)
        ).to.be.reverted;
      });
      ...
      it("Transferring governance to same address should fail", async function () {
        await expect(
          poolContract
            .connect(currentGovernance)
            .transferGovernance(currentGovernance.address)
        ).to.be.reverted;
      });

      it("Transfer as non governance should fail", async function () {
        await expect(
          poolContract.connect(addrs[0]).transferGovernance(governance2.address)
        ).to.be.reverted;
      });
    });
  });
  ...
  describe("Transfer Protocol Wallet", function () {
    it("Can transfer", async function () {
      await poolContract
        .connect(currentGovernance)
        .transferProtocolWallet(addrs[1].address);
    ...
    describe("Rejection scenarios", function () {
      ...
      it("Transferring to a NULL address should fail", async function () {
        await expect(
          poolContract
            .connect(currentGovernance)
            .transferProtocolWallet(NULL_ADDRESS)
        ).to.be.reverted;
      });
    });
  });
  ...
  describe("Pause", function () {
    ...
    describe("Rejection scenarios", function () {
      it("Pausing when paused should fail", async function () {
        await poolContract.connect(currentGovernance).pause();
        await expect(poolContract.connect(currentGovernance).pause()).to.be
          .reverted;
      });
  ...
  describe("Resume", function () {
    ...
    describe("Rejection scenarios", function () {
      it("Resuming when not paused should fail", async function () {
        await poolContract.connect(currentGovernance).unpause();
        await expect(poolContract.connect(currentGovernance).unpause()).to.be
          .reverted;
      });
