...
describe("SaplingPool", function () {
  ...
  describe("Staking", function () {
    ...
    it("Manager can stake", async function () {
      ...
      expect(await tokenContract.balanceOf(manager.address)).to.equal(
        balanceBefore.sub(stakeAmount)
      );
    });
    ...
    it("Stake is reflected on the pool contract balance", async function () {
      ...
      expect(balance)
        .to.equal(prevBalance.add(stakeAmount))
    ...
    it("Manager can stake on a failed pool and have a correct pool balance", async function () {
      ...
      let applicationId = BigNumber.from(
    ...
    describe("Rejection scenarios", function () {
      ...
      it("Staking when the pool is paused should fail", async function () {
        ...
        await tokenContract
          .connect(manager)
          .approve(poolContract.address, stakeAmount);
        await expect(poolContract.connect(manager).stake(stakeAmount)).to.be
          .reverted;
      });
      ...
      it("Staking as the protocol should fail", async function () {
        await tokenContract
          .connect(manager)
          .transfer(protocol.address, stakeAmount);
        await tokenContract
          .connect(protocol)
          .approve(poolContract.address, stakeAmount);
        await expect(poolContract.connect(protocol).stake(stakeAmount)).to.be
          .reverted;
      });
      ...
      it("Staking as the governance should fail", async function () {
        await tokenContract
          .connect(manager)
          .transfer(governance.address, stakeAmount);
        await tokenContract
          .connect(governance)
          .approve(poolContract.address, stakeAmount);
        await expect(poolContract.connect(governance).stake(stakeAmount)).to.be
          .reverted;
      });
      ...
      it("Staking as a lender should fail", async function () {
        await tokenContract
          .connect(manager)
          .transfer(lender1.address, stakeAmount);
        await tokenContract
          .connect(lender1)
          .approve(poolContract.address, stakeAmount);
        await expect(poolContract.connect(lender1).stake(stakeAmount)).to.be
          .reverted;
      });
      ...
      it("Staking as a borrower should fail", async function () {
        ...
        await tokenContract
          .connect(borrower1)
          .approve(poolContract.address, stakeAmount);
        await expect(poolContract.connect(borrower1).stake(stakeAmount)).to.be
          .reverted;
      });
    });
  });
  ...
  describe("Unstaking", function () {
    ...
    it("Manager can unstake", async function () {
      ...
      expect(await tokenContract.balanceOf(manager.address)).to.equal(
        balanceBefore.add(unstakeAmount).sub(exitFee)
      );
    });
    ...
    describe("Amount Unstakable", function () {
      it("Can view amount unstakable", async function () {
        ...
        expect(await poolContract.amountUnstakable()).to.equal(
          expectedUnstakable
        );
      });
    ...
    it("Unstaking is reflected on the pool contract balance", async function () {
      ...
      expect(balance)
        .to.equal(
          prevBalance
            .sub(unstakeAmount)
            .add(unstakeAmount.mul(exitFeePercent).div(ONE_HUNDRED_PERCENT))
        )
        .and.equal(await poolContract.tokenBalance());
    });
    ...
    describe("Rejection scenarios", function () {
      ...
      it("Unstaking an amount greater than unstakable should fail", async function () {
        ...
        await expect(
          poolContract.connect(manager).unstake(amountUnstakable.add(1))
        ).to.be.reverted;
      });

      it("Unstaking when the pool is paused should fail", async function () {
        await poolContract.connect(governance).pause();
        await expect(poolContract.connect(manager).unstake(unstakeAmount)).to.be
          .reverted;
      });

      it("Unstaking as the protocol should fail", async function () {
        await expect(poolContract.connect(protocol).unstake(unstakeAmount)).to
          .be.reverted;
      });

      it("Unstaking as the governance should fail", async function () {
        await expect(poolContract.connect(governance).unstake(unstakeAmount)).to
          .be.reverted;
      });

      it("Unstaking as a lender should fail", async function () {
        await expect(poolContract.connect(lender1).unstake(unstakeAmount)).to.be
          .reverted;
      });
      ...
      it("Unstaking as a borrower should fail", async function () {
        ...
        await expect(poolContract.connect(borrower1).unstake(unstakeAmount)).to
          .be.reverted;
      });
    });
  });
  ...
  describe("Deposits", function () {
    ...
    describe("Amount depositable", function () {
      it("Can view amount depositable", async function () {
        ...
        expect(await poolContract.amountDepositable()).to.equal(
      ...
      it("Amount depositable is zero when pool is full", async function () {
        ...
        await tokenContract
          .connect(lender1)
          .approve(poolContract.address, calculatedDepositable);
    ...
    it("Lender can deposit", async function () {
      ...
      await tokenContract
        ...
        .approve(poolContract.address, depositAmount);
      await poolContract.connect(lender1).deposit(depositAmount);
      expect(await poolContract.balanceOf(lender1.address)).to.equal(
        depositAmount
      );

      expect(await tokenContract.balanceOf(lender1.address)).to.equal(
        balanceBefore.sub(depositAmount)
      );
    });
    ...
    describe("Rejection scenarios", function () {
      ...
      it("Depositing an amount greater than allowed should fail", async function () {
        ...
        await tokenContract
          .connect(lender1)
          .approve(poolContract.address, amountDepositable.add(1));
        await expect(
          poolContract.connect(lender1).deposit(amountDepositable.add(1))
        ).to.be.reverted;
      });
      ...
      it("Depositing when the pool is paused should fail", async function () {
        ...
        await tokenContract
          .connect(lender1)
          .approve(poolContract.address, depositAmount);
        await expect(poolContract.connect(lender1).deposit(depositAmount)).to.be
          .reverted;
      });
      ...
      it("Depositing as the manager should fail", async function () {
        await tokenContract
          .connect(manager)
          .approve(poolContract.address, depositAmount);
        await expect(poolContract.connect(manager).deposit(depositAmount)).to.be
          .reverted;
      });
      ...
      it("Depositing as the protocol should fail", async function () {
        await tokenContract
          .connect(lender1)
          .transfer(protocol.address, depositAmount);
        await tokenContract
          .connect(protocol)
          .approve(poolContract.address, depositAmount);
        await expect(poolContract.connect(protocol).deposit(depositAmount)).to
          .be.reverted;
      });
      ...
      it("Depositing as the governance should fail", async function () {
        await tokenContract
          .connect(lender1)
          .transfer(governance.address, depositAmount);
        await tokenContract
          .connect(governance)
          .approve(poolContract.address, depositAmount);
        await expect(poolContract.connect(governance).deposit(depositAmount)).to
          .be.reverted;
      });
    });
  });
  ...
  describe("Withdrawals", function () {
    ...
    it("Early Withdraw should charge an exit fee", async function () {
      ...
      let expectedWithdrawalFee = withdrawAmount
        .mul(exitFeePercent)
    ...
    describe("Rejection scenarios", function () {
      ...
      it("Withdrawing an amount greater than lender's balance should fail", async function () {
        ...
        await expect(poolContract.connect(lender1).withdraw(balance.add(1))).to
          .be.reverted;
      });
      ...
      it("Withdrawing an amount greater than available should fail", async function () {
        ...
        let amountWithdrawable = await poolContract.amountWithdrawable(
          lender1.address
        );

        await expect(
          poolContract.connect(lender1).withdraw(amountWithdrawable.add(1))
        ).to.be.reverted;
      });

      it("Withdrawing when the pool is paused should fail", async function () {
        await poolContract.connect(governance).pause();
        await expect(poolContract.connect(lender1).withdraw(withdrawAmount)).to
          .be.reverted;
      });

      it("Withdrawing as the manager should fail", async function () {
        let balance = await poolContract.balanceStaked();
        await expect(poolContract.connect(manager).withdraw(balance.div(10))).to
          .be.reverted;
      });
      ...
      it("Withdrawing as a borrower should fail", async function () {
        ...
        await expect(poolContract.connect(borrower2).withdraw(loanAmount)).to.be
          .reverted;
      });
    });
    ...
    describe("Protocol fees", function () {
      ...
      it("Protocol can withdraw earned protocol fees", async function () {
        let tokenBalanceBefore = await tokenContract.balanceOf(
          protocol.address
        );
        let poolBalanceBefore = await poolContract.protocolEarningsOf(
          protocol.address
        );

        await poolContract.connect(protocol).withdrawProtocolEarnings();

        expect(await tokenContract.balanceOf(protocol.address)).to.equal(
      ...
      it("When a new protocol wallet address is set, earned protocol fees are allocated to the new address", async function () {
        let oldProtocolBalanceBefore = await poolContract.protocolEarningsOf(
          protocol.address
        );
        let newProtocolBalanceBefore = await poolContract.protocolEarningsOf(
          addrs[0].address
        );

        await poolContract
          .connect(governance)
          .transferProtocolWallet(addrs[0].address);

        expect(
          await poolContract.protocolEarningsOf(protocol.address)
        ).to.equal(0);
        expect(
          await poolContract.protocolEarningsOf(addrs[0].address)
        ).to.equal(newProtocolBalanceBefore.add(oldProtocolBalanceBefore));
      });
      ...
      it("Protocol fee withdrawal is reflected on the pool contract balance", async function () {
        ...
        let withdrawAmount = await poolContract.protocolEarningsOf(
          protocol.address
        );
        await poolContract.connect(protocol).withdrawProtocolEarnings();
      ...
      describe("Rejection scenarios", function () {
        it("Protocol fees cannot be withdrawn while the pool is paused", async function () {
          await poolContract.connect(governance).pause();
          await expect(
            poolContract.connect(protocol).withdrawProtocolEarnings()
          ).to.be.reverted;
        });
  ...
  describe("Projected APY", function () {
    ...
    it("Can view lender APY given current pool state", async function () {
      ...
      let currentStakePercent =
        ONE_HUNDRED_PERCENT / poolFunds.div(stakeAmount).toNumber();
      let managerEarningsPercent =
        (currentStakePercent * (managersEarnFactor - ONE_HUNDRED_PERCENT)) /
        ONE_HUNDRED_PERCENT;
      let managerWithdrawableAPY = remainingAPY
        .mul(managerEarningsPercent)
        .div(managerEarningsPercent + ONE_HUNDRED_PERCENT);

      let expectedLenderAPY = remainingAPY
        .sub(managerWithdrawableAPY)
        .toNumber();
    ...
    it("Can view projected lender APY", async function () {
      ...
      let borrowRate = loanAmount
        .mul(ONE_HUNDRED_PERCENT)
        .div(poolFunds)
        .toNumber();

      expect(await poolContract.projectedLenderAPY(borrowRate, apr)).to.equal(
        expectedLenderAPY
      );
    });
    ...
    it("Increase in borrow rate is linearly reflected on projected lender APY within margin of integer math accuracy", async function () {
      ...
      // pool APY
      let poolAPY = BigNumber.from(apr)
        .mul(projectedBorrowAmount)
        .div(poolFunds);
      ...
      let managerWithdrawableAPY =
        ...
        (managerEarningsPercent *
          (ONE_HUNDRED_PERCENT - protocolEarningPercent)) /
          ONE_HUNDRED_PERCENT;

      let expectedLenderAPY = poolAPY
        .sub(protocolAPY)
        .sub(managerWithdrawableAPY)
        .toNumber();

      let borrowRate = projectedBorrowAmount
        .mul(ONE_HUNDRED_PERCENT)
        .div(poolFunds)
        .toNumber();

      expect(
        (await poolContract.projectedLenderAPY(borrowRate * 2, apr)) -
          expectedLenderAPY * 2
      ).to.lte(10);
      expect(
        (await poolContract.projectedLenderAPY(borrowRate * 3, apr)) -
          expectedLenderAPY * 3
      ).to.lte(10);
    });
    ...
    describe("Rejection scenarios", function () {
      it("APY projection should fail when borrow rate of over 100% is requested", async function () {
        ...
        let ONE_HUNDRED_PERCENT = await poolContract.ONE_HUNDRED_PERCENT();
        await expect(
          poolContract.projectedLenderAPY(ONE_HUNDRED_PERCENT + 1, apr)
        ).to.be.reverted;
      });
    });
  });
