...
describe("Lender (SaplingPool)", function () {
  ...
  beforeEach(async function () {
    ...
    let verificationHub = await (
      await ethers.getContractFactory("VerificationHub")
    ).deploy(manager.address, protocol.address);

    let tokenFactory = await (
      await ethers.getContractFactory("TokenFactory")
    ).deploy();
    let loanDeskFactory = await (
      await ethers.getContractFactory("LoanDeskFactory")
    ).deploy();
    let poolFactory = await (
    ...
    let poolAddress = poolContractTx.events.filter(
      (e) => e.event === "LendingPoolReady"
    )[0].args["pool"];
    ...
    await tokenContract
      .connect(manager)
      .approve(poolContract.address, stakeAmount);
    await poolContract.connect(manager).stake(stakeAmount);

    await tokenContract
      .connect(lender1)
      .approve(poolContract.address, depositAmount);
  ...
  describe("Loan Request", function () {
    ...
    it("Borrower can request a loan", async function () {
      ...
      let applicationId = (await requestLoanTx.wait()).events.filter(
        (e) => e.event === "LoanRequested"
      )[0].args.applicationId;
    ...
    it("Can view most recent applicationId by address", async function () {
      ...
      expect(
        (await loanDesk.borrowerStats(borrower1.address)).recentApplicationId
      ).to.equal(applicationId);
    });
    ...
    describe("Borrower Statistics", function () {
      it("Loan requests increments all time request count", async function () {
        let prevCountRequested = (
          await loanDesk.borrowerStats(borrower1.address)
        ).countRequested;
        ...
        expect(
          (await loanDesk.borrowerStats(borrower1.address)).countRequested
        ).to.equal(prevCountRequested.add(1));
      });
    });
  });
  ...
  describe("Approve/Deny Loans", function () {
    ...
    beforeEach(async function () {
      ...
      applicationId = (await loanDesk.borrowerStats(borrower1.address))
        .recentApplicationId;
    ...
    describe("Offer", function () {
      it("Manager can offer loans", async function () {
        ...
        expect(
          (await loanDesk.loanApplications(applicationId)).status
        ).to.equal(LoanApplicationStatus.OFFER_MADE);
        expect((await loanDesk.loanOffers(applicationId)).offeredTime).to.equal(
          blockTimestamp
        );
      });
      ...
      describe("Rejection scenarios", function () {
        ...
        it("Offering a loan with an amount greater than available liquidity should fail", async function () {
          ...
          let targetLiquidityPercent =
            await poolContract.targetLiquidityPercent();
          ...
          let otherApplicationId = (
            await loanDesk.borrowerStats(borrower2.address)
          ).recentApplicationId;
          let otherApplication = await loanDesk.loanApplications(
            otherApplicationId
          );
        ...
        it("Offering a loan while pool stake is insufficient should fail", async function () {
          ...
          let otherLoanId = (
            await poolContract.borrowerStats(borrower2.address)
          ).recentLoanId;
    ...
    describe("Deny", function () {
      it("manager can deny loans", async function () {
        ...
        expect(
          (await loanDesk.loanApplications(applicationId)).status
        ).to.equal(LoanApplicationStatus.DENIED);
      });
      ...
      describe("Rejection scenarios", function () {
        it("Denying a loan that is not in APPLIED status should fail", async function () {
          ...
          await expect(loanDesk.connect(manager).denyLoan(applicationId)).to.be
            .reverted;
        });

        it("Denying a nonexistent loan should fail", async function () {
          await expect(loanDesk.connect(manager).denyLoan(applicationId.add(1)))
            .to.be.reverted;
        });

        it("Denying a loan as the protocol should fail", async function () {
          await expect(loanDesk.connect(protocol).denyLoan(applicationId)).to.be
            .reverted;
        });

        it("Denying a loan as the governance should fail", async function () {
          await expect(loanDesk.connect(governance).denyLoan(applicationId)).to
            .be.reverted;
        });

        it("Denying a loan as a lender should fail", async function () {
          await expect(loanDesk.connect(lender1).denyLoan(applicationId)).to.be
            .reverted;
        });

        it("Denying a loan as the borrower should fail", async function () {
          await expect(loanDesk.connect(borrower1).denyLoan(applicationId)).to
            .be.reverted;
        });

        it("Denying a loan from an unrelated address should fail", async function () {
          await expect(loanDesk.connect(addrs[0]).denyLoan(applicationId)).to.be
            .reverted;
        });
      });
  ...
  describe("Borrow/Cancel Loans", function () {
    ...
    describe("Borrow", function () {
      it("Borrowers can borrow", async function () {
        ...
        let loanId = (await poolContract.borrowerStats(borrower1.address))
          .recentLoanId;

        let loan = await poolContract.loans(loanId);
        expect(loan.status).to.equal(LoanStatus.OUTSTANDING);

        expect(await tokenContract.balanceOf(borrower1.address)).to.equal(
          balanceBefore.add(loan.amount)
        );
      });
      ...
      describe("Rejection scenarios", function () {
        it("Borrowing a loan that is not in APPROVED status should fail", async function () {
          ...
          await expect(poolContract.connect(borrower1).borrow(applicationId)).to
            .be.reverted;
        });
        ...
        it("Borrowing a nonexistent loan should fail", async function () {
          await expect(
            poolContract.connect(lender1).borrow(applicationId.add(1))
          ).to.be.reverted;
        });

        it("Borrowing a loan as the protocol should fail", async function () {
          await expect(poolContract.connect(protocol).borrow(applicationId)).to
            .be.reverted;
        });

        it("Borrowing a loan as the governance should fail", async function () {
          await expect(poolContract.connect(governance).borrow(applicationId))
            .to.be.reverted;
        });

        it("Borrowing a loan as a lender should fail", async function () {
          await expect(poolContract.connect(lender1).borrow(applicationId)).to
            .be.reverted;
        });

        it("Borrowing a loan from an unrelated address should fail", async function () {
          await expect(poolContract.connect(addrs[0]).borrow(applicationId)).to
            .be.reverted;
        });
      });
      ...
      describe("Borrower Statistics", function () {
        it("Borrowing a loan increases amount borrowed", async function () {
          let prevAmountBorrowed = (
            await poolContract.borrowerStats(borrower1.address)
          ).amountBorrowed;
          ...
          expect(stat.amountBorrowed).to.equal(
            prevAmountBorrowed.add(loan.amount)
          );
        });
        ...
        it("Borrowing a loan increments outstanding loan count", async function () {
          ...
          let stat = await poolContract.borrowerStats(borrower1.address);
          expect(stat.countOutstanding).to.equal(
            prevStat.countOutstanding.add(1)
          );
        });
      });
    });
    ...
    describe("Cancel", function () {
      it("Manager can cancel", async function () {
        expect(
          await loanDesk.canCancel(applicationId, manager.address)
        ).to.equal(true);
        await loanDesk.connect(manager).cancelLoan(applicationId);
        expect(
          (await loanDesk.loanApplications(applicationId)).status
        ).to.equal(LoanApplicationStatus.OFFER_CANCELLED);
      });
      ...
      it("Manager can cancel while other loans are present (Updating weighted avg loan APR", async function () {
        ...
        let otherApplicationId = BigNumber.from(
          (await requestLoanTx.wait()).events[0].data
        );
      ...
      describe("Rejection scenarios", function () {
        it("Cancelling a loan that is not in APPROVED status should fail", async function () {
          ...
          await expect(loanDesk.connect(manager).cancelLoan(applicationId)).to
            .be.reverted;
        });

        it("Cancelling a nonexistent loan should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId.add(1), manager.address)
          ).to.equal(false);
          await expect(
            loanDesk.connect(manager).cancelLoan(applicationId.add(1))
          ).to.be.reverted;
        });

        it("Cancelling a loan as the protocol should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId, protocol.address)
          ).to.equal(false);
          await expect(loanDesk.connect(protocol).cancelLoan(applicationId)).to
            .be.reverted;
        });

        it("Cancelling a loan as the governance should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId, governance.address)
          ).to.equal(false);
          await expect(loanDesk.connect(governance).cancelLoan(applicationId))
            .to.be.reverted;
        });

        it("Cancelling a loan as a lender should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId, lender1.address)
          ).to.equal(false);
          await expect(loanDesk.connect(lender1).cancelLoan(applicationId)).to
            .be.reverted;
        });

        it("Cancelling a loan as the borrower should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId, borrower1.address)
          ).to.equal(false);
          await expect(loanDesk.connect(borrower1).cancelLoan(applicationId)).to
            .be.reverted;
        });

        it("Cancelling a loan from an unrelated address should fail", async function () {
          expect(
            await loanDesk.canCancel(applicationId, addrs[0].address)
          ).to.equal(false);
          await expect(loanDesk.connect(addrs[0]).cancelLoan(applicationId)).to
            .be.reverted;
        });
      });
      ...
      describe("Cancelling a loan on inactive manager", function () {
        beforeEach(async function () {
          let inactivityPeriod =
            await poolContract.MANAGER_INACTIVITY_GRACE_PERIOD();
        ...
        describe("Rejection scenarios", function () {
          it("A lender without sufficient balance can't cancel", async function () {
            expect(
              await loanDesk.canCancel(applicationId, lender2.address)
            ).to.equal(false);
            await expect(loanDesk.connect(lender2).cancelLoan(applicationId)).to
              .be.reverted;
          });
          ...
          it("Borrower can't cancel", async function () {
            ...
            await expect(loanDesk.connect(borrower1).cancelLoan(applicationId))
              .to.be.reverted;
          });
          ...
          it("An unrelated address can't cancel", async function () {
            ...
            await expect(loanDesk.connect(addrs[0]).cancelLoan(applicationId))
              .to.be.reverted;
          });
        });
      });
  ...
  describe("Repay/Default Loans", function () {
    ...
    beforeEach(async function () {
      ...
      let applicationId = (await loanDesk.borrowerStats(borrower1.address))
        .recentApplicationId;
      ...
      await poolContract.connect(borrower1).borrow(applicationId);
      loanId = (await poolContract.borrowerStats(borrower1.address))
        .recentLoanId;
    });
    ...
    describe("Repay", function () {
      it("Borrower can do a partial payment", async function () {
        ...
        await tokenContract
          .connect(borrower1)
          .approve(poolContract.address, paymentAmount);
        ...
        expect(await tokenContract.balanceOf(borrower1.address)).to.equal(
          balanceBefore.sub(paymentAmount)
        );
      });
      ...
      it("Borrower can do full payments", async function () {
        ...
        expect(loanDetail.totalAmountRepaid).to.equal(paymentAmount);
        expect(loanDetail.lastPaymentTime).to.equal(blockTimestamp);
        expect((await poolContract.loans(loanId)).status).to.equal(
          LoanStatus.REPAID
        );
      ...
      it("3rd party can do a partial payment on behalf of the borrower", async function () {
        ...
        await poolContract
          .connect(lender3)
          .repayOnBehalf(loanId, paymentAmount, borrower1.address);
        let blockTimestamp = await (await ethers.provider.getBlock()).timestamp;
        ...
        expect(await tokenContract.balanceOf(lender3.address)).to.equal(
          balanceBefore.sub(paymentAmount)
        );
      });
      ...
      it("Repaying a loan will allocate protocol fees to the protocol", async function () {
        let balanceBefore = await poolContract.protocolEarningsOf(
          protocol.address
        );
        let loan = await poolContract.loans(loanId);
        ...
        let loanDetail = await poolContract.loanDetails(loanId);
        let protocolEarningPercent =
          await poolContract.protocolEarningPercent();
        let ONE_HUNDRED_PERCENT = await poolContract.ONE_HUNDRED_PERCENT();
        ...
        expect(
          await poolContract.protocolEarningsOf(protocol.address)
        ).to.equal(balanceBefore.add(expectedProtocolFee));
      });
      ...
      it("Repaying a loan will allocate protocol fees to the manager", async function () {
        ...
        let managerExcessLeverageComponent =
          (await poolContract.managerEarnFactor()) - ONE_HUNDRED_PERCENT;

        let currentStakePercent = stakedShares
        ...
        expect(await poolContract.protocolEarningsOf(manager.address)).to.equal(
          balanceBefore.add(managerEarnedInterest)
        );
      });

      it("Overpaying a loan should only charge up to total amount due", async function () {
        let prevWalletBalance = await tokenContract.balanceOf(
          borrower1.address
        );

        let loanBalanceDue = await poolContract.loanBalanceDue(loanId);
        let paymentAmount = loanBalanceDue.add(
          BigNumber.from(500).mul(TOKEN_MULTIPLIER)
        );
      ...
      it("Borrower can do a payment with amount less than the required minimum but equal to outstanding balance", async function () {
        ...
        let paymentAmount1 = (await poolContract.loanBalanceDue(loanId)).sub(
          paymentAmount2
        );

        await tokenContract
          .connect(borrower1)
          .approve(poolContract.address, paymentAmount1.add(paymentAmount2));
        await poolContract.connect(borrower1).repay(loanId, paymentAmount1);

        await expect(
          poolContract.connect(borrower1).repay(loanId, paymentAmount2)
        ).to.be.ok;
      ...
      describe("Rejection scenarios", function () {
        it("Repaying a less than minimum payment amount on a loan with a greater outstanding balance should fail", async function () {
          ...
          await tokenContract
            ...
            .approve(poolContract.address, paymentAmount);
          await expect(
            poolContract.connect(protocol).repay(loanId, paymentAmount)
          ).to.be.reverted;
        });
        ...
        it("Repaying less than the outstanding balance on a loan with balance less than the minimum required should fail", async function () {
          ...
          await expect(
            poolContract.connect(borrower1).repay(loanId, paymentAmount2.sub(1))
          ).to.be.reverted;
        });
        ...
        it("Repaying a loan that is not in OUTSTANDING status should fail", async function () {
          ...
          await tokenContract
            .connect(borrower1)
            .approve(poolContract.address, loanAmount);
          await expect(
            poolContract.connect(borrower1).repay(loanId, paymentAmount)
          ).to.be.reverted;
        });
        ...
        it("Repaying a nonexistent loan should fail", async function () {
          await tokenContract
            ...
            .approve(poolContract.address, loanAmount);
          await expect(
            poolContract.connect(borrower1).repay(loanId.add(1), loanAmount)
          ).to.be.reverted;
        });
        ...
        it("Repaying a loan as the protocol should fail", async function () {
          ...
          await tokenContract
            .connect(manager)
            .transfer(protocol.address, paymentAmount);
          await tokenContract
            .connect(protocol)
            .approve(poolContract.address, paymentAmount);
          await expect(
        ...
        it("Repaying a loan as the governance should fail", async function () {
          ...
          await tokenContract
            .connect(manager)
            .transfer(governance.address, paymentAmount);
          await tokenContract
            .connect(governance)
            .approve(poolContract.address, paymentAmount);
          await expect(
            poolContract.connect(governance).repay(loanId, paymentAmount)
          ).to.be.reverted;
        });
        ...
        it("Repaying a loan from an unrelated address should fail", async function () {
          ...
          await tokenContract
            .connect(manager)
            .transfer(addrs[0].address, paymentAmount);
          await tokenContract
            .connect(addrs[0])
            .approve(poolContract.address, paymentAmount);
          await expect(
            poolContract.connect(addrs[0]).repay(loanId, paymentAmount)
          ).to.be.reverted;
        });
        ...
        it("Repaying a loan on behalf of a wrong borrower should fail", async function () {
          ...
          await expect(
            poolContract
              .connect(lender3)
              .repayOnBehalf(loanId, loanAmount, borrower2.address)
          ).to.be.reverted;
        });
      });
      ...
      describe("Borrower Statistics", function () {
        describe("On Full Repay", function () {
          ...
          it("Fully repaying a loan decrements outstanding loan count", async function () {
            expect(stat.countOutstanding).to.equal(
              prevStat.countOutstanding.sub(1)
            );
          });
        ...
        describe("On Partial Repay", function () {
          ...
          beforeEach(async function () {
            ...
            prevLoanDetail = await poolContract.loanDetails(loanId);
            let paymentAmount = (await poolContract.loanBalanceDue(loanId)).div(
              2
            );
    ...
    describe("Default", function () {
      describe("Default before the grace period", function () {
        describe("Rejection scenarios", function () {
          it("Defaulting a loan before the grace period is up should fail", async function () {
            await expect(poolContract.connect(manager).defaultLoan(loanId)).to
              .be.reverted;
          });
        });
      });
      ...
      describe("Default after grace period", function () {
        ...
        it("Manager can default a partially repaid loan", async function () {
          ...
          expect(
            await poolContract.canDefault(loanId, manager.address)
          ).to.equal(true);
          ...
          expect(await poolContract.poolFunds()).to.equal(
            poolFundsBefore.sub(lossAmount)
          );
          expect(await poolContract.balanceStaked()).to.equal(
            stakedBalanceBefore.sub(lossAmount)
          );
        });
        ...
        it("Manager can default a loan with an loss amount equal to the managers stake", async function () {
          ...
          expect(
            await poolContract.canDefault(otherLoanId, manager.address)
          ).to.equal(true);
          await poolContract.connect(manager).defaultLoan(otherLoanId);
        ...
        describe("Rejection scenarios", function () {
          ...
          it("Defaulting a nonexistent loan should fail", async function () {
            expect(
              await poolContract.canDefault(loanId.add(1), manager.address)
            ).to.equal(false);
            await expect(
              poolContract.connect(manager).defaultLoan(loanId.add(1))
            ).to.be.reverted;
          });

          it("Defaulting a loan as the protocol should fail", async function () {
            expect(
              await poolContract.canDefault(loanId, protocol.address)
            ).to.equal(false);
            await expect(poolContract.connect(protocol).defaultLoan(loanId)).to
              .be.reverted;
          });

          it("Defaulting a loan as the governance should fail", async function () {
            expect(
              await poolContract.canDefault(loanId, governance.address)
            ).to.equal(false);
            await expect(poolContract.connect(governance).defaultLoan(loanId))
              .to.be.reverted;
          });

          it("Defaulting a loan as a lender should fail", async function () {
            expect(
              await poolContract.canDefault(loanId, lender1.address)
            ).to.equal(false);
            await expect(poolContract.connect(lender1).defaultLoan(loanId)).to
              .be.reverted;
          });

          it("Defaulting a loan as the borrower should fail", async function () {
            expect(
              await poolContract.canDefault(loanId, borrower1.address)
            ).to.equal(false);
            await expect(poolContract.connect(borrower1).defaultLoan(loanId)).to
              .be.reverted;
          });

          it("Defaulting a loan from an unrelated address should fail", async function () {
            expect(
              await poolContract.canDefault(loanId, addrs[0].address)
            ).to.equal(false);
            await expect(poolContract.connect(addrs[0]).defaultLoan(loanId)).to
              .be.reverted;
          });
        });
        ...
        describe("Defaulting a loan on inactive manager", function () {
          ...
          describe("Rejection scenarios", function () {
            it("A lender without sufficient balance can't default", async function () {
              expect(
                await poolContract.canDefault(loanId, lender2.address)
              ).to.equal(false);
              await expect(poolContract.connect(lender2).defaultLoan(loanId)).to
                .be.reverted;
            });
            ...
            it("Borrower can't default", async function () {
              ...
              await expect(poolContract.connect(borrower1).defaultLoan(loanId))
                .to.be.reverted;
            });
            ...
            it("An unrelated address can't default", async function () {
              ...
              await expect(poolContract.connect(addrs[0]).defaultLoan(loanId))
                .to.be.reverted;
            });
          });
        });
        ...
        describe("Borrower Statistics", function () {
          describe("On Full Default", function () {
            ...
            it("Full default increments all time default count", async function () {
              expect(stat.countDefaulted).to.equal(
                prevStat.countDefaulted.add(1)
              );
            });
            ...
            it("Full default removes loan amount from borrowed amount", async function () {
              expect(stat.amountBorrowed).to.equal(
                prevStat.amountBorrowed.sub(loan.amount)
              );
            });
          ...
          describe("On Partial Default", function () {
            ...
            beforeEach(async function () {
              ...
              let paymentAmount = (
                await poolContract.loanBalanceDue(loanId)
              ).div(2);
              await tokenContract
                .connect(borrower1)
                .approve(poolContract.address, paymentAmount);
